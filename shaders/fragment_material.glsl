...
void main()
{
    ...
    //FragColor = phongShadingBumpMapping(texture(texture_diffuse, TexCoords), L, E, normal_tangentspace);
    if(has_height){
        if(has_diffuse)
            FragColor = phongShading(texture(texture_diffuse, TexCoords));
        else {
    ...
    //~ FragColor = vec4(0.5);
    //~ if(light.type == 0) FragColor = vec4(1.0);
    //~ else if(light.type == 1){ //only texture or diffuse
        if(has_diffuse)
